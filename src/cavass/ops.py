...
def execute_cavass_cmd(cavass_cmd):
...
def copy_pose(input_file_1, input_file_2, output_file):
    ...
    try:
        execute_cavass_cmd(f'copy_pose "{input_file_1}" "{input_file_2}" "{output_file}"')
    except Exception as e:
...
def save_cavass_file(output_file,
    ...
    Args:
        output_file (str):
        data (numpy.ndarray):
        binary (bool): save as binary data if True.
        ...
        pose_reference_file (str or None, optional, default=None): if `pose_reference_file` is given, copy pose
    ...
    if binary:
        ...
        if pose_reference_file is None:
            try:
                execute_cavass_cmd(f'importMath "{tmp_mat}" matlab "{tmp_file}" {size} {spacing}')
                execute_cavass_cmd(f'ndthreshold "{tmp_file}" "{output_file}" 0 1 1')
            except Exception as e:
                if made_output_dir and os.path.isdir(output_dir):
                    shutil.rmtree(output_dir)
            ...
            try:
                execute_cavass_cmd(f'importMath "{tmp_mat}" matlab "{tmp_file}" {size}')
                tmp_file1 = os.path.join(output_dir, f'tmp_{uuid.uuid1()}.BIM')
                tmp_files.append(tmp_file1)
                execute_cavass_cmd(f'ndthreshold "{tmp_file}" "{tmp_file1}" 0 1 1')
        ...
        if pose_reference_file is None:
            try:
                execute_cavass_cmd(f'importMath "{tmp_mat}" matlab "{output_file}" {size} {spacing}')
            except Exception as e:
                if made_output_dir and os.path.isdir(output_dir):
                    shutil.rmtree(output_dir)
                for tmp_file in tmp_files:
...
def median2d(input_file, output_file, mode=0):
    ...
    try:
        execute_cavass_cmd(f'median2d {input_file} {output_file} {mode}')
    except Exception as e:
        if made_output_dir and os.path.isdir(output_dir):
            shutil.rmtree(output_dir)
        if os.path.exists(output_file):
...
def export_math(input_file, output_file, output_file_type='matlab', first_slice=-1, last_slice=-1):
    ...
    try:
        execute_cavass_cmd(f'exportMath {input_file} {output_file_type} {output_file} {first_slice} {last_slice}')
    except Exception as e:
        if made_output_dir and os.path.isdir(output_dir):
            shutil.rmtree(output_dir)
        if os.path.exists(output_file):
...
def render_surface(input_bim_file, output_file):
    ...
    try:
        execute_cavass_cmd(ndinterpolate_cmd)
    except Exception as e:
        if made_output_dir and os.path.isdir(output_dir):
            shutil.rmtree(output_dir)
    ...
    try:
        execute_cavass_cmd(gaussian_cmd)
    except Exception as e:
        if made_output_dir and os.path.isdir(output_dir):
            shutil.rmtree(output_dir)
        if os.path.exists(interpl_tmp_bim_file):
